        ...
        <div class="card border" style="min-width: 25rem;">
            ...
            <div class="card-body text-success">
                <div class="d-flex flex-column">
                    ...
                    @if(isset($count_subject))
                        <input type="text" value="{{ $subject->id }}" name="subject_id" id="subject_id" class="form-control" hidden>
                        <div class="">
                            <label for="title" class="form-label">Subject title: {{ $count_subject->title }} </label>
                        </div>
                    @else
                        <div>
                            <select name="subject_id" id="subject_id" class="form-select" aria-label="Default select example">
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }} @selected(isset($count_subject) && $subject == $count_subject)">{{ $subject->title }}</option>
                                    {{ $user->name }}
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="">
                        ...
                        <input type=""  name="grade" id="grade" class="form-control" placeholder="{{ isset($count_subject) ?  $count_subject->pivot->grade : '' }}">
                    </div>
